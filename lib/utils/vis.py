...
def save_debug_images(config, input_data, meta, target, joints_pred, output, prefix):
    if not config.DEBUG.DEBUG:
        return

    if config.DEBUG.SAVE_BATCH_IMAGES_GT:
        save_batch_image_with_joints(
            input_data, meta["joints"], meta["joints_vis"], "{}_gt.jpg".format(prefix)
        )

    if config.DEBUG.SAVE_BATCH_IMAGES_PRED:
        save_batch_image_with_joints(
            input_data, joints_pred, meta["joints_vis"], "{}_pred.jpg".format(prefix)
        )

    # if config.LOSS.TYPE == "heatmap":
    #     if config.DEBUG.SAVE_HEATMAPS_GT:
    #         save_batch_heatmaps(input_data, target, '{}_hm_gt.jpg'.format(prefix))
    #
    #     if config.DEBUG.SAVE_HEATMAPS_PRED:
    #         save_batch_heatmaps(input_data, output, '{}_hm_pred.jpg'.format(prefix))
...
def save_batch_image_with_joints(
    ...
    ndarr = cv2.cvtColor(ndarr, cv2.COLOR_RGB2BGR)  # RGB to BGR
