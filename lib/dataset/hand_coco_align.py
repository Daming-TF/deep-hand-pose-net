...
class HandCOCODatasetAlign(Dataset):
    ...
    def generate_target(self, joints, joints_vis):
        ...
        for joint_id in range(self.num_joints):
            target_reg[joint_id, 0] = joints[joint_id, 0] / self.image_size[0] - 0.5
    ...
    def __getitem__(self, idx):
        ...
        #         shift_x = np.clip(np.random.randn(), -1., 1.) * tf
        #         shift_y = np.clip(np.random.randn(), -1., 1.) * tf
        #         shift = np.array([shift_x, shift_y])
        #
        #     if self.flip and random.random() <= 0.5:
        #         # Flip horizontal
        #         data_numpy = data_numpy[:, ::-1, :]                     # image horizontal-flip
        #         joints[:, 0] = data_numpy.shape[1] - joints[:, 0] - 1   # landmark horizontal-flip
        #         c[0] = data_numpy.shape[1] - c[0] - 1
        #
        #     if self.color_jitter and random.random() < 0.5:
        #         max_b = 45
        #         brightness_adjust = np.clip(np.random.randn() * max_b, -max_b*2, max_b*2)
        #         max_c = 0.2
        #         contrast_adjust = np.clip(np.random.randn() * max_c + 1, 1-max_c, 1+max_c)
        #         data_numpy = adjust_contrast_brightness(data_numpy, contrast_adjust, brightness_adjust)
        #
        ...
        # ################################################################################################################
        # if idx < 30:
        #     cv2.imwrite(f"/workspace/nas-data/Codes/H0097-deep-hand-pose/deep-hand-pose-net/test/o_{str(idx)}.jpg",
        #                 cv2.cvtColor(data_numpy, cv2.COLOR_RGB2BGR).astype(np.uint8))

        alignmenter = HandInfo(img_size=self.image_size[0])
        input_data, warp_matrix = alignmenter(data_numpy, ori_joints)
        # if idx < 30:
        #     cv2.imwrite(f"/workspace/nas-data/Codes/H0097-deep-hand-pose/deep-hand-pose-net/test/w_{str(idx)}.jpg",
        #                 cv2.cvtColor(input_data, cv2.COLOR_RGB2BGR).astype(np.uint8))
        ################################################################################################################
        ...
        # landmark affine
        joints = ori_joints.copy()
        for i in range(self.num_joints):
            if joints_vis[i, 0] > 0.0:
                # joints[i, 0:2] = affine_transform(joints[i, 0:2], trans)
                joints[i, 0:2] = affine_transform(ori_joints[i, 0:2], warp_matrix)

        if self.transform:
            input_data = self.transform(input_data)

        target, target_weight, target_reg = self.generate_target(joints, joints_vis)
